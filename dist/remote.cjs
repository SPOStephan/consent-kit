...
function toPlugin(s) {
  switch (s.type) {
    case "google-tag-manager":
      ...
      return consentKit.googleTagManager({ id: s.id, analytics: s.analytics !== false, ads: s.ads !== false });
    case "meta-pixel":
      if (s.loadVia === "gtm") return consentKit.metaPixel({ loadVia: "gtm" });
      return str(s.id) && /^\d+$/.test(s.id) ? consentKit.metaPixel({ id: s.id }) : null;
    case "tiktok-pixel":
      if (s.loadVia === "gtm") return consentKit.tiktokPixel({ loadVia: "gtm" });
      return str(s.id) && /^[A-Z0-9]+$/i.test(s.id) ? consentKit.tiktokPixel({ id: s.id }) : null;
    case "youtube":
      return consentKit.youtube();
    case "google-maps":
      return consentKit.googleMaps();
    default:
      return null;
  }
}
...
exports.fromRemoteConfig = fromRemoteConfig;
